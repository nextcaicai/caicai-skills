#!/usr/bin/env python3
"""Fetch Feishu/Lark document as Markdown using Feishu Open API."""
...
def fetch_feishu_doc(url_or_id):
    ...
    if doc_type == "wiki":
        ...
        if real_id:
            ...
            return None, f"无法获取知识库节点信息: {doc_id}"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: fetch_feishu.py <feishu_url>", file=sys.stderr)
        print("  需要环境变量: FEISHU_APP_ID, FEISHU_APP_SECRET", file=sys.stderr)
        sys.exit(1)

    url = sys.argv[1]
    result, err = fetch_feishu_doc(url)
